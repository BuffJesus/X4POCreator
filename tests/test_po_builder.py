...
class POBuilderTests(unittest.TestCase):
    ...
    def test_scan_folder_hides_legacy_sales_when_detailed_pair_is_found_and_legacy_is_hidden(self):
        class FakeVar:
        ...
        self.assertEqual(fake_app.var_sales_path.get(), "")
    ...
    def test_scan_folder_auto_shows_legacy_sales_when_it_is_the_only_sales_source_found(self):
        ...
        with patch("po_builder.os.path.isdir", return_value=True), patch(
            "po_builder.parsers.scan_directory_for_reports",
            return_value={"sales": "C:\\Reports\\legacy.csv"},
        ), patch("po_builder.ui_load.refresh_load_file_sections") as refresh_sections:
            po_builder.POBuilderApp._scan_folder(fake_app)

        self.assertEqual(fake_app.var_sales_path.get(), "C:\\Reports\\legacy.csv")
        self.assertTrue(fake_app.var_show_legacy_inputs.get())
        refresh_sections.assert_called_once()
        self.assertIn("Part Sales & Receipts", status_calls[-1])
        self.assertIn("only the combined sales report was found", status_calls[-1])

    def test_get_and_set_default_vendor_policy_preset_round_trip(self):
        saved = {}
        fake_session = SimpleNamespace(default_vendor_policy_preset="")
        fake_app = SimpleNamespace(
            app_settings={},
        ...
        self.assertEqual(
            po_builder.POBuilderApp._get_default_vendor_policy_preset(fake_app),
            po_builder.DEFAULT_VENDOR_POLICY_PRESET,
        )

        po_builder.POBuilderApp._set_default_vendor_policy_preset(fake_app, "hybrid_friday_2000")

        self.assertEqual(fake_app.app_settings["default_vendor_policy_preset"], "hybrid_friday_2000")
    ...
    def test_buy_rule_save_explicit_pack_trigger_policy_is_persisted_and_applied(self):
        ...
        saved_rule = fake_app.order_rules["AER-:GH781-4"]
        self.assertEqual(saved_rule["order_policy"], "pack_trigger")
        self.assertTrue(saved_rule["policy_locked"])
        self.assertEqual(saved_rule["reorder_trigger_qty"], 60)
        self.assertEqual(fake_app.filtered_items[0]["order_policy"], "pack_trigger")
        self.assertEqual(fake_app.filtered_items[0]["suggested_qty"], 300)
        self.assertEqual(fake_app.filtered_items[0]["final_qty"], 300)
