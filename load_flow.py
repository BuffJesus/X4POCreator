    ...
    *,
    ...
    if paths.get("po"):
        ...
        try:
            ...
            # v0.9.1: Detect X4 "Draft PO" rows in the listing.  These are
            # unposted lines generated by X4's own auto-max-PO feature; if we
            # counted them as committed, PO Builder would silently skip every
            # item X4 already drafted.  Surface the count so the operator
            # knows they are being filtered out of the reorder trigger.
            draft_po_rows = [
                po for po in po_items
                if str(po.get("po_type", "") or "").strip().lower() == "draft po"
            ]
            if draft_po_rows:
                draft_qty_total = 0.0
                for po in draft_po_rows:
                    try:
                        draft_qty_total += float(po.get("qty", 0) or 0)
                    except (TypeError, ValueError):
                        pass
                draft_keys = {(po["line_code"], po["item_code"]) for po in draft_po_rows}
                sample = "\n".join(
                    f"  {lc}/{ic}"
                    for lc, ic in sorted(draft_keys)[:6]
                )
                more = len(draft_keys) - 6
                if more > 0:
                    sample += f"\n  ... +{more} more"
                warnings.append((
                    "Draft PO Detected",
                    (
                        f"{len(draft_po_rows)} 'Draft PO' line(s) "
                        f"({len(draft_keys)} unique item(s), total qty {draft_qty_total:g}) "
                        f"found in the PO Part Listing.\n\n"
                        "These are unposted draft lines from X4's auto-max PO "
                        "feature. PO Builder treats them as NOT yet committed, "
                        "so the same items can still appear in your reorder "
                        "suggestions. If you want draft POs to count as on-order, "
                        "set \"exclude_draft_pos_from_committed\": false in "
                        "po_builder_settings.json.\n\n"
                        f"Sample items:\n{sample}"
                    ),
                ))
                for lc, ic in sorted(draft_keys):
                    startup_warning_rows.append({
                        "warning_type": "Draft PO Detected",
                        "severity": "info",
                        "line_code": lc,
                        "item_code": ic,
                        ...
                        "reference_date": "",
                        "qty": "",
                        "po_reference": "Draft PO (excluded from committed qty)",
                        "details": "X4 auto-max draft line; not counted as on-order.",
                    })
