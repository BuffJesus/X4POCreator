...
# v0.10.0-alpha2: the LOAD_FILE_SECTIONS data definition lives in
# ``ui_load_data.py`` so the Qt build (which excludes tkinter from its
# PyInstaller spec) can import the same source-of-truth constants
# without transitively importing tkinter.  Re-exported here so any
# existing caller that imports from ``ui_load`` keeps working.
from ui_load_data import LOAD_FILE_SECTIONS, iter_load_file_rows  # noqa: F401
