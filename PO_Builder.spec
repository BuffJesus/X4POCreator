...
#
# PyInstaller spec for PO Builder (PySide6).
#
#   build.bat → this spec → dist/POBuilder.exe
#
...
a = Analysis(
    ...
    hooksconfig={},
    runtime_hooks=[],
    # Exclude every unused Qt module to keep the build lean.
    # PyInstaller's dependency scanner only picks up imports that are
    # actually executed, so these excludes are belt-and-suspenders.
    excludes=[
        # Heavy PySide6 submodules we don't use
